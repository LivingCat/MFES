...
public class Store {
  ...
  private Number calculateGrandTotalPurchase(final VDMMap productsList) {
    ...
    Number sum = 0L;
    for (Iterator iterator_31 = MapUtil.dom(Utils.copy(productsList)).iterator();
        iterator_31.hasNext();
        ) {
      Product p = (Product) iterator_31.next();
      sum =
          sum.doubleValue()
              + p.getBuyPrice().doubleValue() * ((Number) Utils.get(productsList, p)).longValue();
    }
    return sum;
  }
  ...
  private Number calculateGrandTotalSale(final VDMMap productsList) {
    ...
    Number sum = 0L;
    for (Iterator iterator_32 = MapUtil.dom(Utils.copy(productsList)).iterator();
        iterator_32.hasNext();
        ) {
  ...
  public PurchaseFromClient buy(final VDMMap products, final Client client, final Date date) {
    ...
    for (Iterator iterator_33 = MapUtil.dom(Utils.copy(products)).iterator();
        iterator_33.hasNext();
        ) {
      ...
      Utils.mapSeqUpdate(
          productsCpy,
          p,
          ((Number) Utils.get(product, p)).longValue()
              + ((Number) Utils.get(products, p)).longValue());
    }
  ...
  public PurchaseFromSupplier buy(final VDMMap products, final Supplier supplier, final Date date) {
    ...
    VDMMap productsCpy = Utils.copy(products);
    purchase = SetUtil.union(Utils.copy(purchase), SetUtil.set(newPurchase));
    for (Iterator iterator_34 = MapUtil.dom(Utils.copy(products)).iterator();
        iterator_34.hasNext();
        ) {
  ...
  public Sale sell(final VDMMap products, final Client client, final Date date) {
    ...
    for (Iterator iterator_35 = MapUtil.dom(Utils.copy(products)).iterator();
        iterator_35.hasNext();
        ) {
  ...
  public void removeProduct(final String pName) {
    ...
    for (Iterator iterator_36 = MapUtil.dom(Utils.copy(product)).iterator();
        iterator_36.hasNext();
        ) {
  ...
  public void removeProductClass(final String pcName) {
    ...
    for (Iterator iterator_38 = MapUtil.dom(Utils.copy(product)).iterator();
        iterator_38.hasNext();
        ) {
