...
public class StoreTest {
	...
	private void testAddProductClass() {
		...
		for (Iterator iterator_13 = set_13.iterator(); iterator_13.hasNext() && (exists1Counter_2.longValue() < 2L);) {
		...
		for (Iterator iterator_14 = set_14.iterator(); iterator_14.hasNext() && (exists1Counter_3.longValue() < 2L);) {
	...
	private void testRemoveProductClass() {
		...
		for (Iterator iterator_15 = set_15.iterator(); iterator_15.hasNext() && !(existsExpResult_5);) {
		...
		for (Iterator iterator_16 = set_16.iterator(); iterator_16.hasNext() && !(existsExpResult_6);) {
	...
	private void testAddProduct() {
		...
		for (Iterator iterator_17 = set_17.iterator(); iterator_17.hasNext() && (exists1Counter_4.longValue() < 2L);) {
		...
		for (Iterator iterator_39 = MapUtil.dom(Utils.copy(products)).iterator(); iterator_39.hasNext();) {
	...
	private void testRemoveProduct() {
		...
		for (Iterator iterator_18 = set_18.iterator(); iterator_18.hasNext() && !(existsExpResult_7);) {
		...
		for (Iterator iterator_19 = set_19.iterator(); iterator_19.hasNext() && !(existsExpResult_8);) {
	...
	private void testAddStock() {
		...
		products = store.getStoreProducts();
		for (Iterator iterator_40 = MapUtil.dom(Utils.copy(products)).iterator(); iterator_40.hasNext();) {
			Product p = (Product) iterator_40.next();
			if (Utils.equals(p.getName(), p1.getName())) {
				assertEqual(6L, ((Number) Utils.get(products, p)));
			}
		}
		for (Iterator iterator_41 = MapUtil.dom(Utils.copy(products)).iterator(); iterator_41.hasNext();) {
	...
	private void testBuyFromClient() {
		...
		for (Iterator iterator_20 = set_20.iterator(); iterator_20.hasNext() && (exists1Counter_5.longValue() < 2L);) {
		...
		for (Iterator iterator_21 = set_21.iterator(); iterator_21.hasNext() && (exists1Counter_6.longValue() < 2L);) {
		...
		for (Iterator iterator_22 = set_22.iterator(); iterator_22.hasNext() && (exists1Counter_7.longValue() < 2L);) {
			Purchase p = ((Purchase) iterator_22.next());
			if (Utils.equals(SetUtil.intersect(SetUtil.set(p1, p2), MapUtil.dom(p.getProducts())).size(), 2L)) {
				exists1Counter_7++;
			}
		}
		...
		products = store.getStoreProducts();
		for (Iterator iterator_42 = MapUtil.dom(Utils.copy(products)).iterator(); iterator_42.hasNext();) {
	...
	private void testBuyFromSupplier() {
		...
		for (Iterator iterator_23 = set_23.iterator(); iterator_23.hasNext() && (exists1Counter_8.longValue() < 2L);) {
		...
		for (Iterator iterator_24 = set_24.iterator(); iterator_24.hasNext() && (exists1Counter_9.longValue() < 2L);) {
		...
		for (Iterator iterator_25 = set_25.iterator(); iterator_25.hasNext() && (exists1Counter_10.longValue() < 2L);) {
		...
		products = store.getStoreProducts();
		for (Iterator iterator_43 = MapUtil.dom(Utils.copy(products)).iterator(); iterator_43.hasNext();) {
	...
	private void testSell() {
		...
		for (Iterator iterator_26 = set_26.iterator(); iterator_26.hasNext() && (exists1Counter_11.longValue() < 2L);) {
		...
		for (Iterator iterator_27 = set_27.iterator(); iterator_27.hasNext() && (exists1Counter_12.longValue() < 2L);) {
		...
		for (Iterator iterator_28 = set_28.iterator(); iterator_28.hasNext() && (exists1Counter_13.longValue() < 2L);) {
			Sale s = ((Sale) iterator_28.next());
			if (Utils.equals(SetUtil.intersect(SetUtil.set(p1, p2), MapUtil.dom(s.getProducts())).size(), 2L)) {
				exists1Counter_13++;
			}
		}
		...
		products = store.getStoreProducts();
		for (Iterator iterator_44 = MapUtil.dom(Utils.copy(products)).iterator(); iterator_44.hasNext();) {
	...
	private void testReportMetrics() {

		Store store = new Store("Kids2Kids", "PT", 1000L);
		ProductClass pc1 = new ProductClass("toy");
		Product p1 = new Product(pc1, "doll", 10L, 15L);
		Product p2 = new Product(pc1, "car", 15L, 20L);
		VDMMap sold = MapUtil.map(new Maplet(p1, 2L), new Maplet(p2, 1L));
		VDMMap bought = MapUtil.map(new Maplet(p1, 3L), new Maplet(p2, 2L));
		Client client = new Client("joao");
		Supplier supplier = new Supplier("catarina");
		Date date = new Date(30L, 12L, 2018L);
		Sale sale = null;
		Purchase purchase = null;
		store.addProductClass(pc1);
		store.addProduct(p1, 10L);
		store.addProduct(p2, 10L);
		assertEqual(store.getAssets(), 350L);
		sale = store.sell(Utils.copy(sold), client, date);
		purchase = store.buy(Utils.copy(bought), supplier, date);
		purchase = store.buy(Utils.copy(bought), client, date);
		assertEqual(store.getNumberOfSales(), 1L);
		assertEqual(store.getNumberOfPurchases(), 2L);
		assertEqual(store.getNumberOfClientPurchases(), 1L);
		assertEqual(store.getNumberOfSupplierPurchases(), 1L);
		assertEqual(store.getNetSales(), 50L);
		assertEqual(store.getCostOfGoodsSold(), 35L);
		assertEqual(store.getGrossProfit(), 15L);
	}
